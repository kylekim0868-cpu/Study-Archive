...
static Widget *widget_new(const VTable *vt, int id, const char *label) {
    ...
    */
    // Widget 크기의 공간을 동적으로 할당하고, 그 주소를 포인터 변수 w에 저장
    Widget *w = malloc(sizeof *w);
    if (!w) { perror("malloc"); exit(1); }

    // 동적으로 할당된 힙 메모리에 있는 Widget객체의 멤버 변수를 대입
    // w는 Widget을 바라보는 포인터일 뿐. 주소 예시) 0x1000라고 가정한다면, w = 0x1000 일뿐
    w->vtbl = vt;
    w->id = id;
    w->closed = 0;
    // strncpy의 의미는?
...
int main(void) {
    Screen s = { .count = 0 }; // Screen 구조체 s라는 변수의 멤버 변수 count = 0 으로 명시적으로 초기화하고 나머지도 초기화
    ...
    // Closed Widget을 정리하는 코드
    // Screen의 count만큼 순회
    for(int i=0; i<s.count; i++){
        // Widget을 가리키는 포인터 주소를 담기 w (지역 변수)
        Widget *w = s.items[i];
        // Closed = 1인 items 찾기
        if(w->closed == 1){
            widget_destroy(w); // 해당 메모리(Widget 객체) free
            s.items[i] = NULL; // free된 메모리를 가리키는 포인터 NULL 초기화
            // NULL 원소는 맨 뒤로
            Widget *temp = s.items[i];
            s.items[i] = s.items[s.count-1]; 
            s.items[s.count-1] = temp;
            s.count -= 1; // 줄어든 count수 줄이기
        }
    }
